...
| Q (17) | **요청ID** | `Code.gs` (중복 저장 방지) | `COL_REQID` |

> 자격증현황 시트도 같은 목적으로 **I열(9) 요청ID** 를 쓴다 (`CERT_COL_REQID`).
...
#### 배포 버전 스탬프 (2026-09-09)
"지금 돌아가는 게 이 코드가 맞나?" 를 1초에 확인하려고 넣었다. 하루에 세 번 이 문제로 잘못 판단했다(백업이 v1이었던 calendar.gs, CDN 캐시로 두 번).
```
GET ?type=version → { code, notion, calendar, at }
```
- `Code.gs` `CODE_VERSION` · `notion.gs` `NOTION_VERSION` · `calendar.gs` `CAL_VERSION` · `schedule.html` `PAGE_VERSION`
- 페이지 푸터(`#verStamp`)에 **page / code / notion / cal** 이 나란히 찍힌다. `page ≠ code` 면 붉게 표시된다.
- ⚠️ **코드를 고쳐 재배포할 때마다 해당 파일의 버전 문자열을 함께 올릴 것.** 안 올리면 이 장치가 무의미해진다.
- `typeof` 로 감싸 두어 파일 하나가 없어도 doGet 이 죽지 않는다.

#### 같은 저장이 두 번 들어가는 것 방지 — 멱등키 (2026-09-09)
네트워크가 흔들려 **응답만** 유실되면 페이지가 다시 보낼 수 있고, 그러면 행이 두 번 생겼다. 그래서 예전에는 `add`·`complete`·`certAdd` 를 아예 재시도하지 못했다(저장 실패 = 사용자가 다시 눌러야 함).
- 페이지가 요청마다 `reqId` 를 만들어 보낸다(`newReqId()`), **재시도해도 같은 reqId 를 쓴다**.
- 서버가 `강의스케쥴 Q열` / `자격증현황 I열` 에 기록하고, 같은 reqId 가 이미 있으면 새로 만들지 않고 **그때 만든 결과(같은 id)를 그대로 돌려준다**.
- `complete` 는 K열='Y' 로, `delete` 는 "없으면 성공"으로 멱등이 된다.
- 덕분에 `RETRYABLE` 화이트리스트를 없애고 **모든 액션을 재시도**한다.
- 검증: 응답 유실 재시도 시 행 1개 유지·같은 id 반환, 다른 reqId 는 정상 생성, complete 재시도 시 경력사항 1행 유지, delete 재시도 시 오류 아님.
...
### ⚠️ 홈 화면 앱 = 웹페이지 그대로
아이폰 홈 화면의 스케쥴 앱은 **서비스워커가 없는 그냥 전체화면 웹페이지**다. 거기서 추가·수정하는 것은 PC 브라우저와 **완전히 동일한 경로**(페이지 → 시트 → 캘린더)를 탄다.
- 따라서 "폰에서 고친 일정"이라는 개념은 **아이폰 캘린더 앱에서 직접 고친 경우만** 해당하고, 사용자는 그렇게 쓰지 않는다(개인 일정만 아이폰 캘린더에 넣는다).
- 2026-09-09에 '캘린더 외부 수정 감지' 기능을 넣었다가 **쓰지 않는 기능이라 다시 제거**했다. 필요해지면 그때 다시 만든다.
- 개인 일정은 애초에 안전하다: ① `getCalendarsByName('강의 스케쥴')` 로 전용 캘린더만 연다 ② 정리 루프가 `if (!tag) continue;` 로 태그 없는 일정을 건너뛴다.
- 캐시된 옛 페이지를 붙들고 있는지는 **푸터 버전 스탬프**(`page` vs `code`)로 확인한다.
